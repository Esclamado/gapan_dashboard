...
class Save extends \Lns\Sb\Controller\Controller {
    ...
    public function run(){
        ...
        if($payload['error'] == 1){
            ...
            $hasRecord = $this->_cart->getByColumn(['user_id'=>$userId], 1);
            if($hasRecord){
                $save = $this->_orders;
                ...
                if ($orderId) {
                    ...
                    $carts = $this->_cart->getByColumn(['user_id' => $userId], 0);
                    if ($carts) {
                        foreach ($carts as $cart) {
                            if (in_array($cart->getData('id'), $cartIds)) {
                                $save = $this->_orderitems;
                                $save->setData('order_id', $orderId);
                                $save->setData('type_id', $cart->getData('type_id')); 
                                $orderitemsId = $save->__save();

                                $price = $this->_price->getByColumn(['type_id' => $cart->getData('type_id')], 1);
                                ...
                                if ($orderitemsId) {
                                    ...
                                    foreach ($cartdetails as $cartdetail) {
                                        ...
                                        $entity->setData('type_id', $cartdetail->getData('type_id'));
                                        $entity->setData('qty', $cartdetail->getData('qty'));
                                        $entity->setData('price', $price);
                                        $orderitemdetailsId = $entity->__save();

                                        /* DELETE Add to Cart - START */
                                        if($orderitemdetailsId){
                                            $cartRecord = $this->_cart->getByColumn([/* 'user_id' => $userId */'id' => $cart->getData('id')], 1);
                                            if ($cartRecord) {
