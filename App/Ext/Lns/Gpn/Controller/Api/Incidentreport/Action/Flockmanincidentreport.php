...
class Flockmanincidentreport extends \Lns\Sb\Controller\Controller {
    ...
    public function run() {
		...
		if ($payload['error'] == 1) {
            ...
            $photo = $this->upload($userId, 'flockman');
            if ($photo) {
                ...
                if ($save) {
                    ...
                    $pushData = $record->getData();
                    $pushData['house'] = $house;
                    $pushData['recordStatus'] = 'Pending Inspector Resolve';

                    $message = "<strong>".$flockman."</strong> filed an incident report for house no. ".$house['house_name'] . ".";
                    
                    $this->_notification->setNotification((int)$record->getData('id'), $userId, 3, null, $message, $this->getPost('receiver_id'));
                    
                    $to = $this->_deviceToken->getDeviceTokenById($this->getPost('receiver_id'));
                    if ($to) {
                        $title = "Incident Report";
                        $message = $flockman. " filed an incident report for house no. ".$house['house_name'] . ".";
                        $content = "Content Here";
                        $api_key = $this->_siteConfig->getData('site_fcm_key');
                        $pushAction = "daily_house_harvest_file_incident_report";
                        $this->_push->sendNotif($to->getData('token'), $title, $message, $content, $api_key, $pushAction);

                        /* send notif to inspector */
                        $this->_cfs->save($this->getPost('receiver_id'), $flockman, $message, 'daily_house_harvest_file_incident_report', $this->_siteConfig->getData('site_firebase_project_id'), $this->_siteConfig->getData('site_firebase_web_api_key'), json_encode($pushData));
                    }
                    $action = "<strong>" . $flockman . "</strong> filed an incident report for house no. " . $house['house_name'] . ".";
                    $this->_audittrail->saveAudittrail($userId, $flockman, $action, 'incident_report_submit_flockman');
