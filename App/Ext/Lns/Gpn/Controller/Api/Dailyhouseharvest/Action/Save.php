...
/*             $hasRecord = $this->_dailyHouseHarvest->hasRecord($this->getParam('house_id')); */
                ...
                if($entity){
                    ...
                    if ($entity->getData('prepared_by') == $userId) {
                        ...
                        if($entity->getData('checked_by')){
                            $bird_count = (int) $entity->getData('bird_count');
               ...
               /* if ($hasRecord) {
                ...
                if ($save) {
                    ...
                    $this->jsonData['data']['recordStatus'] = $this->_incidentReport->getIncidentReport((int)$userId, (int)$data->getData('prepared_by'), (int)$data->getData('checked_by'), (int)$data->getData('received_by'), (int)$data->getData('id'), 3);
                    if ($this->getParam('prepared_by')) {
                        ...
                        if ($inspectors) {
                            ...
                            $action = "<strong>" . $flockman . "</strong> sent a daily house report for house no. " . $house['house_name'] . ".";
                            $this->_audittrail->saveAudittrail($userId, $flockman, $action, 'daily_house_report_submit');
                        }
                        /* send notif to inspector */
                        ...
                        if ($managers) {
                            foreach ($managers as $manager) {
                                ...
                                $flockman = $this->_userProfile->getFullNameById($userId);
                                $message = $flockman." sent you a daily house report for house no. ".$house['house_name'] . ".";
                                $this->_notification->setNotification((int)$data->getData('id'), $userId, 1, null, $message, $manager->getData('id'));
                                $to = $this->_deviceToken->getDeviceTokenById($manager->getData('id'));
                                $pushData['recordStatus'] = 'Pending';
                                $this->_cfs->save($manager->getData('id'), $flockman, $message, 'daily_house_harvest', $this->_siteConfig->getData('site_firebase_project_id'), $this->_siteConfig->getData('site_firebase_web_api_key'), json_encode($pushData));
                            }
                        }
                        /* send notif to manager */
                    }
                    if ($this->getParam('checked_by')) {
                        ...
                        $action = "<strong>" . $inspector . "</strong> approved a daily report for house/building no. " . $house['house_name'] . ".";
                        $this->_audittrail->saveAudittrail($userId, $inspector, $action, 'daily_house_report_approve');
                        ...
                        if ($managers) {
                            foreach ($managers as $manager) {
                                ...
                                $inspector = $this->_userProfile->getFullNameById($userId);
                                $message = $inspector." approved a daily report for house/building no. ".$house['house_name'].".";
                                $this->_notification->setNotification($data->getData('id'), $userId, 5, null, $message, $manager->getData('id'));
                                $to = $this->_deviceToken->getDeviceTokenById($manager->getData('id'));
                                $pushData['recordStatus'] = 'Pending';
                                $this->_cfs->save($manager->getData('id'), $inspector, $message, 'daily_house_harvest_success', $this->_siteConfig->getData('site_firebase_project_id'), $this->_siteConfig->getData('site_firebase_web_api_key'), json_encode($pushData));
                            }
                        }
                        /* send notif to manager */
                    ...
                    if ($this->getParam('received_by')) {
                        ...
                        $action = "<strong>" . $sorter . "</strong> received the daily house report for house/building no. " . $house['house_name'] . ".";
                        $this->_audittrail->saveAudittrail($userId, $sorter, $action, 'daily_house_report_receive');
                        ...
                        if ($managers) {
                            foreach ($managers as $manager) {
                                ...
                                $sorter = $this->_userProfile->getFullNameById($userId);
                                $message = $sorter." received the daily house report for house/building no. ".$house['house_name'].".";
                                $this->_notification->setNotification($data->getData('id'), $userId, 6, null, $message, $manager->getData('id'));
                                $to = $this->_deviceToken->getDeviceTokenById($manager->getData('id'));
                                $pushData['recordStatus'] = 'Received';
                                $this->_cfs->save($manager->getData('id'), $sorter, $message, 'daily_house_harvest_received', $this->_siteConfig->getData('site_firebase_project_id'), $this->_siteConfig->getData('site_firebase_web_api_key'), json_encode($pushData));
                            }
                        }
                        /* send notif to manager */
                    }
                    if($this->getParam('prepared_by')){
                        $this->jsonData['message'] = 'Daily house report sent!';
                    }
                    if ($this->getParam('checked_by')) {
                        $this->jsonData['message'] = 'Daily house report approved!';
                    }
                    if ($this->getParam('received_by')) {
                        $this->jsonData['message'] = 'Daily house report received!';
                    }
                    $this->jsonData['error'] = 0;
                }else{
